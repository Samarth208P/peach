...
export default function HistoryPage() {
  ...
  const userHistory = cancellationEvents?.data.filter((event) => {
    ...
    return payload && (payload.sender === currentAccount?.address || payload.receiver === currentAccount?.address);
  }) || [];
        ...
        {isPending ? (
          ...
          <div className="space-y-3 relative z-10">
            {userHistory.map((tx) => {
              ...
              const settledAmount = (Number(data.receiver_settled_sui) / 1e9).toFixed(2);
              const refundedAmount = (Number(data.sender_refunded_sui) / 1e9).toFixed(2);
